...
/**
 * PostMessageOriginDetector
 * Security analyzer for CWE-345: window.addEventListener('message') without event.origin verification
 */
@Slf4j
@Component
public class PostMessageOriginDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-345-POSTMESSAGEORIGINDETECTOR";
    private static final String CWE_ID = "CWE-345";
    ...
    private static final String DESCRIPTION = "window.addEventListener('message') without event.origin verification";

    private final Pattern pattern = Pattern.compile("(?i)(postmessageorigin|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("PostMessageOrigin Vulnerability")
