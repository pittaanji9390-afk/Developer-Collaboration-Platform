...
/**
 * InsecureDirectObjectRefDetector
 * Security analyzer for CWE-639: Repository settings mutation accessible without admin permission check
 */
@Slf4j
@Component
public class InsecureDirectObjectRefDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-639-INSECUREDIRECTOBJECTREFDETECTOR";
    private static final String CWE_ID = "CWE-639";
    ...
    private static final String DESCRIPTION = "Repository settings mutation accessible without admin permission check";

    private final Pattern pattern = Pattern.compile("(?i)(insecuredirectobjectref|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("InsecureDirectObjectRef Vulnerability")
