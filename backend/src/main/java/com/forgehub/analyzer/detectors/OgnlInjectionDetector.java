 ...
 * Security analyzer for CWE-94: OGNL expression parsing on user controlled parameters
 */
@Slf4j
@Component
public class OgnlInjectionDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-94-OGNLINJECTIONDETECTOR";
    ...
    private static final String DESCRIPTION = "OGNL expression parsing on user controlled parameters";

    private final Pattern pattern = Pattern.compile("(?i)(ognlinjection|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("OgnlInjection Vulnerability")
