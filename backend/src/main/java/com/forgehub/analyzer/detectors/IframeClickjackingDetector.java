 ...
 * Security analyzer for CWE-1021: Missing X-Frame-Options or frame-ancestors CSP directive
 */
@Slf4j
@Component
public class IframeClickjackingDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-1021-IFRAMECLICKJACKINGDETECTOR";
    ...
    private static final String DESCRIPTION = "Missing X-Frame-Options or frame-ancestors CSP directive";

    private final Pattern pattern = Pattern.compile("(?i)(iframeclickjacking|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("IframeClickjacking Vulnerability")
