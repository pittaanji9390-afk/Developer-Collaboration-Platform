...
/**
 * DockerRootUserDetector
 * Security analyzer for CWE-250: Dockerfile missing USER directive executing commands as root
 */
@Slf4j
@Component
public class DockerRootUserDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-250-DOCKERROOTUSERDETECTOR";
    ...
    private static final String DESCRIPTION = "Dockerfile missing USER directive executing commands as root";

    private final Pattern pattern = Pattern.compile("(?i)(dockerrootuser|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("DockerRootUser Vulnerability")
