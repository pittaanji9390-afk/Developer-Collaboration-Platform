...
/**
 * MyBatisSqlInjectionDetector
 * Security analyzer for CWE-89: MyBatis ${...} string substitution causing SQL injection instead of #{...}
 */
@Slf4j
@Component
public class MyBatisSqlInjectionDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-89-MYBATISSQLINJECTIONDETECTOR";
    ...
    private static final String DESCRIPTION = "MyBatis ${...} string substitution causing SQL injection instead of #{...}";

    private final Pattern pattern = Pattern.compile("(?i)(mybatissqlinjection|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("MyBatisSqlInjection Vulnerability")
