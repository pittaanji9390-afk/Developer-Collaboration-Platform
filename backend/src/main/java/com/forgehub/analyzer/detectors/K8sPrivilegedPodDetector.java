...
/**
 * K8sPrivilegedPodDetector
 * Security analyzer for CWE-250: Kubernetes pod spec setting privileged: true
 */
@Slf4j
@Component
public class K8sPrivilegedPodDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-250-K8SPRIVILEGEDPODDETECTOR";
    ...
    private static final String DESCRIPTION = "Kubernetes pod spec setting privileged: true";

    private final Pattern pattern = Pattern.compile("(?i)(k8sprivilegedpod|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("K8sPrivilegedPod Vulnerability")
