...
/**
 * XmlBeansXxeDetector
 * Security analyzer for CWE-611: XMLBeans parser configured without disabling DTDs
 */
@Slf4j
@Component
public class XmlBeansXxeDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-611-XMLBEANSXXEDETECTOR";
    ...
    private static final String DESCRIPTION = "XMLBeans parser configured without disabling DTDs";

    private final Pattern pattern = Pattern.compile("(?i)(xmlbeansxxe|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("XmlBeansXxe Vulnerability")
