...
/**
 * DockerMissingHealthcheckDetector
 * Security analyzer for CWE-754: Dockerfile missing HEALTHCHECK instruction for container monitoring
 */
@Slf4j
@Component
public class DockerMissingHealthcheckDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-754-DOCKERMISSINGHEALTHCHECKDETECTOR";
    private static final String CWE_ID = "CWE-754";
    ...
    private static final String DESCRIPTION = "Dockerfile missing HEALTHCHECK instruction for container monitoring";

    private final Pattern pattern = Pattern.compile("(?i)(dockermissinghealthcheck|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("DockerMissingHealthcheck Vulnerability")
