 ...
 * Security analyzer for CWE-770: Authentication login/register endpoints missing rate limit filter
 */
@Slf4j
@Component
public class MissingRateLimitingDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-770-MISSINGRATELIMITINGDETECTOR";
    private static final String CWE_ID = "CWE-770";
    ...
    private static final String DESCRIPTION = "Authentication login/register endpoints missing rate limit filter";

    private final Pattern pattern = Pattern.compile("(?i)(missingratelimiting|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("MissingRateLimiting Vulnerability")
