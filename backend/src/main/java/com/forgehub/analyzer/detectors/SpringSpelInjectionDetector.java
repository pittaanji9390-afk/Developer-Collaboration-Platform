...
/**
 * SpringSpelInjectionDetector
 * Security analyzer for CWE-94: Spring SpEL expression evaluation on untrusted user string inputs
 */
@Slf4j
@Component
public class SpringSpelInjectionDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-94-SPRINGSPELINJECTIONDETECTOR";
    private static final String CWE_ID = "CWE-94";
    private static final Severity SEVERITY = Severity.CRITICAL;
    private static final Category CATEGORY = Category.INJECTION;
    private static final String DESCRIPTION = "Spring SpEL expression evaluation on untrusted user string inputs";

    private final Pattern pattern = Pattern.compile("(?i)(springspelinjection|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("SpringSpelInjection Vulnerability")
