...
/**
 * PredictableSessionIdDetector
 * Security analyzer for CWE-331: Low entropy session token generation
 */
@Slf4j
@Component
public class PredictableSessionIdDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-331-PREDICTABLESESSIONIDDETECTOR";
    private static final String CWE_ID = "CWE-331";
    private static final Severity SEVERITY = Severity.HIGH;
    private static final Category CATEGORY = Category.BROKEN_AUTH;
    private static final String DESCRIPTION = "Low entropy session token generation";

    private final Pattern pattern = Pattern.compile("(?i)(predictablesessionid|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("PredictableSessionId Vulnerability")
