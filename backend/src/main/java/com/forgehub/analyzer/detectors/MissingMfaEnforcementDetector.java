...
/**
 * MissingMfaEnforcementDetector
 * Security analyzer for CWE-308: Critical administrative endpoints missing multi-factor verification gate
 */
@Slf4j
@Component
public class MissingMfaEnforcementDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-308-MISSINGMFAENFORCEMENTDETECTOR";
    private static final String CWE_ID = "CWE-308";
    private static final Severity SEVERITY = Severity.HIGH;
    private static final Category CATEGORY = Category.BROKEN_AUTH;
    private static final String DESCRIPTION = "Critical administrative endpoints missing multi-factor verification gate";

    private final Pattern pattern = Pattern.compile("(?i)(missingmfaenforcement|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("MissingMfaEnforcement Vulnerability")
