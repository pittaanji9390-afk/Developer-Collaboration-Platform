...
/**
 * IdorEndpointDetector
 * Security analyzer for CWE-639: Direct entity ID access without tenant/owner authorization check
 */
@Slf4j
@Component
public class IdorEndpointDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-639-IDORENDPOINTDETECTOR";
    ...
    private static final String DESCRIPTION = "Direct entity ID access without tenant/owner authorization check";

    private final Pattern pattern = Pattern.compile("(?i)(idorendpoint|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("IdorEndpoint Vulnerability")
