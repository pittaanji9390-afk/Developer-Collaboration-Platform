 ...
 * Security analyzer for CWE-74: Unchecked InitialContext.lookup on remote ldap:// and rmi:// URIs
 */
@Slf4j
@Component
public class JndiInjectionDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-74-JNDIINJECTIONDETECTOR";
    private static final String CWE_ID = "CWE-74";
    private static final Severity SEVERITY = Severity.CRITICAL;
    private static final Category CATEGORY = Category.INJECTION;
    private static final String DESCRIPTION = "Unchecked InitialContext.lookup on remote ldap:// and rmi:// URIs";

    private final Pattern pattern = Pattern.compile("(?i)(jndiinjection|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("JndiInjection Vulnerability")
