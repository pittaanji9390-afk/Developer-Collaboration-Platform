...
/**
 * SaxParserXxeDetector
 * Security analyzer for CWE-611: SAXParserFactory without external general entities feature disabled
 */
@Slf4j
@Component
public class SaxParserXxeDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-611-SAXPARSERXXEDETECTOR";
    private static final String CWE_ID = "CWE-611";
    private static final Severity SEVERITY = Severity.HIGH;
    private static final Category CATEGORY = Category.XXE;
    private static final String DESCRIPTION = "SAXParserFactory without external general entities feature disabled";

    private final Pattern pattern = Pattern.compile("(?i)(saxparserxxe|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("SaxParserXxe Vulnerability")
