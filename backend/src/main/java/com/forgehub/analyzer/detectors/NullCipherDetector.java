...
/**
 * NullCipherDetector
 * Security analyzer for CWE-327: NullCipher instantiation disabling data encryption
 */
@Slf4j
@Component
public class NullCipherDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-327-NULLCIPHERDETECTOR";
    ...
    private static final String DESCRIPTION = "NullCipher instantiation disabling data encryption";

    private final Pattern pattern = Pattern.compile("(?i)(nullcipher|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("NullCipher Vulnerability")
