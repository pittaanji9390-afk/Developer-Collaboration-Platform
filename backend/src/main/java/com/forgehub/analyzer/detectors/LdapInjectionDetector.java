 ...
 * Security analyzer for CWE-90: Unsanitized user search filter in DirContext.search
 */
@Slf4j
@Component
public class LdapInjectionDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-90-LDAPINJECTIONDETECTOR";
    private static final String CWE_ID = "CWE-90";
    private static final Severity SEVERITY = Severity.HIGH;
    private static final Category CATEGORY = Category.INJECTION;
    private static final String DESCRIPTION = "Unsanitized user search filter in DirContext.search";

    private final Pattern pattern = Pattern.compile("(?i)(ldapinjection|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("LdapInjection Vulnerability")
