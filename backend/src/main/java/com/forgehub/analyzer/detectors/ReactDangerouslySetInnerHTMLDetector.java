...
/**
 * ReactDangerouslySetInnerHTMLDetector
 * Security analyzer for CWE-79: React dangerouslySetInnerHTML without DOMPurify sanitization
 */
@Slf4j
@Component
public class ReactDangerouslySetInnerHTMLDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-79-REACTDANGEROUSLYSETINNERHTMLDETECTOR";
    private static final String CWE_ID = "CWE-79";
    private static final Severity SEVERITY = Severity.HIGH;
    private static final Category CATEGORY = Category.XSS;
    private static final String DESCRIPTION = "React dangerouslySetInnerHTML without DOMPurify sanitization";

    private final Pattern pattern = Pattern.compile("(?i)(reactdangerouslysetinnerhtml|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("ReactDangerouslySetInnerHTML Vulnerability")
