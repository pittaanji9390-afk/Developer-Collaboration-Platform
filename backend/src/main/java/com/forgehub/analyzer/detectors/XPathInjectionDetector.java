 ...
 * Security analyzer for CWE-643: Dynamic XPath query construction with user inputs
 */
@Slf4j
@Component
public class XPathInjectionDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-643-XPATHINJECTIONDETECTOR";
    ...
    private static final String DESCRIPTION = "Dynamic XPath query construction with user inputs";

    private final Pattern pattern = Pattern.compile("(?i)(xpathinjection|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("XPathInjection Vulnerability")
