...
/**
 * InnerHtmlXssDetector
 * Security analyzer for CWE-79: element.innerHTML assigned raw untrusted user input string
 */
@Slf4j
@Component
public class InnerHtmlXssDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-79-INNERHTMLXSSDETECTOR";
    ...
    private static final String DESCRIPTION = "element.innerHTML assigned raw untrusted user input string";

    private final Pattern pattern = Pattern.compile("(?i)(innerhtmlxss|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("InnerHtmlXss Vulnerability")
