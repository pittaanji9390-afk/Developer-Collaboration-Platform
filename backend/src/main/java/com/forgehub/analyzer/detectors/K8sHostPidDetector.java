...
/**
 * K8sHostPidDetector
 * Security analyzer for CWE-250: Kubernetes pod spec setting hostPID: true
 */
@Slf4j
@Component
public class K8sHostPidDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-250-K8SHOSTPIDDETECTOR";
    ...
    private static final String DESCRIPTION = "Kubernetes pod spec setting hostPID: true";

    private final Pattern pattern = Pattern.compile("(?i)(k8shostpid|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("K8sHostPid Vulnerability")
