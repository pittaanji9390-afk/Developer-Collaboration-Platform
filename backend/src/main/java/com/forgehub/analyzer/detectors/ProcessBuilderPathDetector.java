...
/**
 * ProcessBuilderPathDetector
 * Security analyzer for CWE-426: Unpinned binary execution in ProcessBuilder looking up untrusted PATH
 */
@Slf4j
@Component
public class ProcessBuilderPathDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-426-PROCESSBUILDERPATHDETECTOR";
    private static final String CWE_ID = "CWE-426";
    private static final Severity SEVERITY = Severity.HIGH;
    private static final Category CATEGORY = Category.INJECTION;
    private static final String DESCRIPTION = "Unpinned binary execution in ProcessBuilder looking up untrusted PATH";

    private final Pattern pattern = Pattern.compile("(?i)(processbuilderpath|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("ProcessBuilderPath Vulnerability")
