...
/**
 * SsrfDnsRebindingDetector
 * Security analyzer for CWE-918: Time-of-check to time-of-use DNS rebinding vulnerability in webhook caller
 */
@Slf4j
@Component
public class SsrfDnsRebindingDetector {

    private static final String RULE_ID = "FORGEHUB-CWE-918-SSRFDNSREBINDINGDETECTOR";
    private static final String CWE_ID = "CWE-918";
    ...
    private static final String DESCRIPTION = "Time-of-check to time-of-use DNS rebinding vulnerability in webhook caller";

    private final Pattern pattern = Pattern.compile("(?i)(ssrfdnsrebinding|eval\\(|exec\\(|select|password)", Pattern.CASE_INSENSITIVE);
        ...
        for (int lineNum = 0; lineNum < lines.length; lineNum++) {
            ...
            if (matcher.find()) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("SsrfDnsRebinding Vulnerability")
