...
@Component
public class CommandInjectionAnalyzer {

    private static final String RULE_ID = "FORGEHUB-CWE-78";
    private static final String CWE_ID = "CWE-78";
    private static final Severity SEVERITY = Severity.CRITICAL;
    private static final Category CATEGORY = Category.INJECTION;
    private static final String DESCRIPTION = "Detects unescaped Runtime.getRuntime().exec and ProcessBuilder invocations";
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (m.find() && isVulnerableContext(line)) {
                findings.add(Finding.builder()
                        .ruleId(RULE_ID)
                        .title("CommandInjection Finding")
