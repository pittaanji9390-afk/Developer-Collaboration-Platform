...
/**
 * SecretMaskingStreamFilter
 * Filters sensitive tokens and passwords from streaming log outputs
 */
@Slf4j
@Component
public class SecretMaskingStreamFilter {

    public PolicyCheckResult evaluate(String subject, Map<String, Object> context) {
        log.debug("Evaluating SecretMaskingStreamFilter on subject: {}", subject);

        return PolicyCheckResult.builder()
                .enforcerName("SecretMaskingStreamFilter")
                .subject(subject)
                .compliant(true)
                .violations(List.of())
