 ...
 * Personal access token creation wizard with granular scope checkboxes
 */
export const PersonalAccessTokensPage: React.FC = () => {
  const [isLoading, setIsLoading] = useState(false);

  return (
    <div className="max-w-7xl mx-auto px-4 py-8 w-full space-y-6" data-testid="personalaccesstokenspage">
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-surface-800">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2.5">
            ...
            <span>PersonalAccessTokens</span>
          </h1>
      ...
      <div className="border border-surface-800 rounded-2xl bg-surface-900 p-6 space-y-4">
        ...
        <p className="text-xs text-slate-300 leading-relaxed">
          Personal access token creation wizard with granular scope checkboxes. Managed through the ForgeHub unified security policy and governance framework.
        </p>
      </div>
    </div>
  );
